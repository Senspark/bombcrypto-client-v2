...
namespace Senspark.Internal {
    internal class AdMobInterstitialAdBridge : IFullScreenAdBridge {
        [NotNull]
        ...
        public AdMobInterstitialAdBridge(
            [NotNull] string id,
        ...
        public async Task<(bool, string)> Load() {
            _ad?.Destroy();
            _ad = null;
            var tcs = new TaskCompletionSource<(InterstitialAd, string)>();
            // ReSharper disable once VariableHidesOuterVariable
            InterstitialAd.Load(_id, new AdRequest(), (ad, error) => {
