...
namespace Senspark {
    [AddComponentMenu("Senspark/Safe Area Layer")]
    public class SafeAreaLayer : MonoBehaviour {
        ...
        private void Refresh() {
            ...
            if (_mirrorEnabled) {
                var inset = Mathf.Max(rect.x, _screenWidth - rect.width - rect.x);
                rect.x = inset;
                rect.width = _screenWidth - inset * 2;
            }
            _safeArea = rect;
            ApplySafeArea(rect);
        }
        ...
        private void ApplySafeArea(Rect rect) {
            ...
            if (_screenWidth > 0 && _screenHeight > 0) {
                ...
                if (anchorMin is { x: >= 0, y: >= 0 } &&
                    anchorMax is { x: >= 0, y: >= 0 }) {
