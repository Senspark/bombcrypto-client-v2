...
namespace Scenes.PvpModeScene.Scripts {
    public class DialogPvpRankDown : Dialog {
        [SerializeField]
        ...
        [SerializeField]
        private TextMeshProUGUI decayRankText;

        private ISoundManager _soundManager;
        
        public static UniTask<DialogPvpRankDown> Create() {
            return ServiceLocator.Instance.Resolve<IPrefabLoaderManager>().Instantiate<DialogPvpRankDown>();
        }
        ...
        public async UniTask SetInfo(PvpRankType rankFrom, PvpRankType rankTo) {
            ...
            decayRankText.gameObject.SetActive(false);
        }
        
        public async void SetInfo(PvpRankType rankType) {
            _soundManager = ServiceLocator.Instance.Resolve<ISoundManager>();
            rankName.text = BLProfileCard.GetRankName(rankType);
            iconRankFrom.sprite = await resource.GetSpriteByPvpRank(rankType);
            iconRankTo.sprite = await resource.GetSpriteByPvpRank(rankType);
            decayRankText.gameObject.SetActive(true);
            SetAnimation();
        }
        ...
        private void PlayAnimation(float y) {
            _soundManager.PlaySound(Audio.RankDown);
            var moveUp = iconRankFrom.transform.DOLocalMoveY(y, 0.3f)
        ...
        private async void DoAnimationUp() {
            ...
            var ani = await resource.GetAnimation(GachaRankChange.RankDown);
