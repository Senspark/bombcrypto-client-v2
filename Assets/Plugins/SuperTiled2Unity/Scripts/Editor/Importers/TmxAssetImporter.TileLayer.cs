...
namespace SuperTiled2Unity.Editor {
    public partial class TmxAssetImporter {
        ...
        private Tilemap GetOrAddTilemapComponent(GameObject go) {
            ...
            var cTilemap = CustomTilemap.Create(go);
            cTilemap.Tiles = m_GlobalTileDatabase.Tiles.Values.ToList();
            var tilemap = cTilemap.Tilemap;
        ...
        private void ProcessLayerDataChunk(GameObject goTilemap, Chunk chunk) {
            ...
            var tilesData = new List<SuperTileData>();
            var dataRaw = new int[chunk.Width, chunk.Height];
            PlaceTiles(goTilemap, chunk, tileIds, tilesData, dataRaw);
            // Add layer data
            var superMap = goTilemap.GetComponentInParent<SuperMap>();
            superMap.AddLayerData(new SuperLayerData() {
                name = m_CurrentTileLayer.m_TiledName,
                x = chunk.X,
                y = chunk.Y,
                width = chunk.Width,
                height = chunk.Height,
        ...
        private void PlaceTiles(GameObject goTilemap, Chunk chunk, List<uint> tileIds, List<SuperTileData> tilesData = null, int[,] dataRaw = null) {
            // Only report each missing tile Id once
            HashSet<int> badTiles = new HashSet<int>();

            var tilemap = goTilemap.GetComponentInParent<CustomTilemap>();
            ...
            for (int i = 0; i < tileIds.Count; i++) {
                ...
                uint utId = tileIds[i];
                if (utId == 0) {
                    if (dataRaw != null) dataRaw[cx, cy] = -1;
                    continue;
                }
                ...
                tilesData?.Add(new SuperTileData() {
                    x = cx,
                    y = cy,
                    tileId = tile.m_TileId
                });
                if (dataRaw != null) dataRaw[cx, cy] = tile.m_TileId;
            }
        }

        private void PlaceTile(GameObject goTilemap, CustomTilemap cTilemap, TilemapRenderer tilemapRenderer, int cx, int cy, SuperTile tile, Vector3Int pos3, TileIdMath tileId) {
            if (m_TilesAsObjects) {
                PlaceTileAsObject(goTilemap, tile, cx, cy, tileId, pos3);
            } else {
                PlaceTileAsTile(goTilemap, cTilemap, tilemapRenderer, tile, tileId, pos3);
            }
        }
        ...
        private void PlaceTileAsTile(GameObject goTilemap, CustomTilemap cTilemap, TilemapRenderer tilemapRenderer, SuperTile tile, TileIdMath tileId, Vector3Int pos3) {
            ...
            if (SuperImportContext.LayerIgnoreMode != LayerIgnoreMode.Visual) {
                ...
                cTilemap.SetTile(pos3, tile);
                cTilemap.Tilemap.SetTransformMatrix(pos3, tile.GetTransformMatrix(tileId.FlipFlags, m_MapComponent.m_Orientation));

#if UNITY_2018_3_OR_NEWER
                if (tilemapRenderer.mode == TilemapRenderer.Mode.Individual) {
                    // Each tile color must be set individually
                    cTilemap.Tilemap.SetColor(pos3, cTilemap.Tilemap.color);
                }
#endif

                cTilemap.Tilemap.SetTileFlags(pos3, TileFlags.LockAll);
            }
