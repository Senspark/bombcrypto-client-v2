...
namespace SuperTiled2Unity.Editor
{
    ...
    public partial class TmxAssetImporter : TiledAssetImporter
    {
            ...
            foreach (XElement xNode in xMap.Elements())
            {
                ...
                using (SuperImportContext.BeginLayerIgnoreMode(ignoreMode))
                {
                    ...
                    else if (xNode.Name == "objectgroup")
                    {
                        ...
                        m_MapComponent.AddObjectGroupData(new ObjectGroupData() {
                           name = xNode.GetAttributeAs("name", ""),
                           objects = ProcessObjectGroupElement(xNode)
                        });
                    }
        ...
        public List<ObjectData> ProcessObjectGroupElement(XElement xObjectGroup) {
            ...
            foreach (var xObject in xObjectGroup.Elements("object")) {
                ...
                d.name = xObject.GetAttributeAs("name", "");
                d.x = xObject.GetAttributeAs("x", 0.0f);
                d.y = xObject.GetAttributeAs("y", 0.0f);
                d.width = xObject.GetAttributeAs("width", 0.0f);
                d.height = xObject.GetAttributeAs("height", 0.0f);
                ...
                if (xPolygon != null) {
                    ...
                    d.type = CollisionShapeType.Polygon;
                    // TODO: update late
                    ...
                    d.type = CollisionShapeType.Polyline;
                    // TODO: update late
                } else if (xObject.Element("ellipse") != null) {
                    // Not support ellipse
                    d.type = CollisionShapeType.Ellipse;
                } else if (xObject.Element("point") != null) {
                    d.type = CollisionShapeType.Point;
                } else {
                    // By default, objects are rectangles
                    d.type = CollisionShapeType.Rectangle;
                }
                objectsData.Add(d);
            }
            return objectsData;
