    ...
    public partial class TilemapEditor : Editor
    {
        ...
        private void DoMapSceneGUI()
        {
            if (m_toggleMapBoundsEdit)
            {
                ...
                var fmh_1026_178_638417833769577170 = Quaternion.identity; Vector3 vMinX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MinGridX * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1027_185_638417833769599190 = Quaternion.identity; Vector3 vMaxX = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2((m_tilemap.MaxGridX + 1f) * m_tilemap.CellSize.x, m_tilemap.MapBounds.center.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1028_178_638417833769601840 = Quaternion.identity; Vector3 vMinY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, m_tilemap.MinGridY * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
                var fmh_1029_185_638417833769604920 = Quaternion.identity; Vector3 vMaxY = Handles.FreeMoveHandle(m_tilemap.transform.TransformPoint(new Vector2(m_tilemap.MapBounds.center.x, (m_tilemap.MaxGridY + 1f) * m_tilemap.CellSize.y)), 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
            ...
            if (EditorWindow.mouseOverWindow == SceneView.currentDrawingSceneView)
            {
                ...
                if (tile != null)
                {
                    ...
                    for (int i = 0; i < handlesPos.Count; ++i)
                    {
                        ...
                        {
                            var fmh_1124_75_638417833769613560 = Quaternion.identity; handlePos = Handles.FreeMoveHandle(handlePos, 0.1f * HandleUtility.GetHandleSize(m_tilemap.transform.position), Vector3.zero, EditorCompatibilityUtils.CubeCap);
