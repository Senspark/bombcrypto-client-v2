...
namespace SpriteTrail {
    ...
    public sealed class SpriteTrail : MonoBehaviour {
        ...
        [Tooltip(
            "The duration of the trail activation (in seconds). After this delay, the trail is automatically disabled")]
        ...
        private TrailElement m_TrailElementTemplate;

        private TrailElement TrailElementTemplate {
            get {
#if UNITY_EDITOR
                ...
                if (m_TrailElementTemplate == null) {
                    var obj = new GameObject(
                        ...
                        typeof(TrailElement));
                    m_TrailElementTemplate = obj.GetComponent<TrailElement>();
                    // DontDestroyOnLoad(m_TrailElementTemplate);
                    m_TrailElementTemplate.transform.SetParent(m_TrailParent);
                    m_TrailElementTemplate.Pool = Pool;
                }
        ...
        public Transform LocalTrailContainer => m_LocalTrailContainer;
        private Transform m_CacheTrailContainer;

        public Transform CacheTrailContainer {
            get {
#if UNITY_EDITOR
                if (!UnityEditor.EditorApplication.isPlaying) {
                    return null;
...
#endif
                if (m_CacheTrailContainer == null) {
                    var container = GameObject.Find("CacheTrailContainer");

                    if (container == null) {
                        m_CacheTrailContainer = new GameObject("CacheTrailContainer").transform;
#if HIDE_TRAIL_IN_HIERRARCHY
                        m_CacheTrailContainer.hideFlags = HideFlags.HideInHierarchy;
#endif
                    } else {
                        m_CacheTrailContainer = container.transform;
                    }

                    // DontDestroyOnLoad(GlobalTrailContainer);
                    m_CacheTrailContainer.transform.SetParent(m_TrailParent);
                }

                return m_CacheTrailContainer;
            }
        }
        ...
        private TrailPool m_TrailPool = new TrailPool();
        public TrailPool Pool => m_TrailPool;
        ...
        private void SetTrailParent(Transform trailParent) {
            if (trailParent == null) {
                return;
            }

            m_TrailParent = trailParent;
            var localTrailContainer = trailParent.Find("LocalTrailContainer");
        ...
        private void EnableTrailEffect(bool forceTrailCreation = true) {
        ...
        public void HideTrail(bool addToFree) {
            ...
            while (m_ElementsInTrail.Count > 0) {
                var e = m_ElementsInTrail.Dequeue();
                if (e != null) {
                    e.Hide(addToFree);
                    if (!addToFree) {
        ...
        private void SpriteTrailInit() {
            SceneManager.sceneUnloaded -= OnSceneUnloaded;
            SceneManager.sceneUnloaded += OnSceneUnloaded;
        }

        private void OnSceneUnloaded(Scene scene) {
            // TrailElementTemplate.RefreshFreeElements();
        }
        ...
        */
        ...
        public void Init(Transform trailParent = null, GameObject objSprite = null) {
            SetTrailParent(trailParent);
            m_CurrentDisplacement = m_ZMoveMax;

            if (objSprite != null) {
                m_SpriteToDuplicate = objSprite.GetComponent<SpriteRenderer>();
            } else if (m_SpriteToDuplicate == null) {
        ...
        private void CalculateCurrentVelocity() {
            if (m_FirstVelocityCheck) {
                m_VelocityLocal = ((Vector2) m_SpriteToDuplicate.transform.localPosition - m_PreviousFrameLocalPos) /
                                  Time.deltaTime;
                m_VelocityWorld = ((Vector2) m_SpriteToDuplicate.transform.position - m_PreviousFrameWorldPos) /
                                  Time.deltaTime;
            }
                ...
                switch (m_CurrentTrailPreset.m_TrailElementSpawnCondition) {
                    case TrailElementSpawnCondition.Time:
                        if (m_PreviousTimeSpawned + m_CurrentTrailPreset.m_TimeBetweenSpawns <= Time.time) {
                            var _TimeError = Time.time -
                                             (m_PreviousTimeSpawned + m_CurrentTrailPreset.m_TimeBetweenSpawns);
                    ...
                    case TrailElementSpawnCondition.Distance:
                        if (m_CurrentTrailPreset.m_DistanceCorrection &&
                            m_CurrentTrailPreset.m_DistanceBetweenSpawns > 0) {
                            ...
                            while (Vector2.Distance(m_PreviousPosSpawned, m_SpriteToDuplicate.transform.position) >=
                                ...
                                var _dir = (Vector2) m_SpriteToDuplicate.transform.position - m_PreviousPosSpawned;
                                var _tmpPos = m_PreviousPosSpawned +
                                              _dir.normalized * m_CurrentTrailPreset.m_DistanceBetweenSpawns;
                                GenerateNewTrailElement(new Vector3(_tmpPos.x, _tmpPos.y,
                                    m_SpriteToDuplicate.transform.position.z));
        ...
        private void GenerateNewTrailElement(Vector3 pos) {
            ...
            var _TmpElement = TrailElementTemplate.GetFreeElement();
            ...
            if (m_TrailParent == null) {
                _TmpElement.m_Transform.SetParent(m_CacheTrailContainer, true);
            } else {
            ...
            _TmpElement.m_myGameObject.layer = m_SpriteToDuplicate.gameObject.layer;
        ...
        private void GenerateNewTrailElement() {
            ...
            _TmpElement.m_Transform.localPosition = Vector3.zero;
            if (m_TrailParent == null) {
                _TmpElement.m_Transform.SetParent(CacheTrailContainer, true);
            } else {
                _TmpElement.m_Transform.SetParent(m_LocalTrailContainer, true);
            }
            var _NewPos = _TmpElement.transform.position;
            _NewPos.z = m_CurrentDisplacement;
            _TmpElement.m_Transform.position = _NewPos;
            _TmpElement.Initialise(this);
            _TmpElement.m_myGameObject.layer = m_SpriteToDuplicate.gameObject.layer;
...
#endif
            var _effectWasEnabled = m_EffectEnabled;
