...
public abstract class SerializableDictionaryBase<TKey, TValue, TValueStorage> : SerializableDictionaryBase, IDictionary<TKey, TValue>, IDictionary, ISerializationCallbackReceiver, IDeserializationCallback, ISerializable
{
	...
	[SerializeField]
    protected TValueStorage[] m_values;
	...
	{
        Debug.Log("SerializableDictionaryBase");
...
[Serializable]
public class SerializableDictionaryEnumKey<TKey, TValue> : SerializableDictionaryBase<TKey, TValue, TValue> where TValue : new() {
    public SerializableDictionaryEnumKey() {}
    public SerializableDictionaryEnumKey(IDictionary<TKey, TValue> dict) : base(dict) {}
    protected SerializableDictionaryEnumKey(SerializationInfo info, StreamingContext context) : base(info, context) {}
    ...
    public override void OnAfterDeserialize()
    {
        ...
        {
            var n = m_keys.Length;
            var mDictTemp = new Dictionary<TKey, TValue>();
            for(var i = 0; i < n; ++i)
            {
                mDictTemp[m_keys[i]] = GetValue(m_values, i);
            }
            m_dict.Clear();
            var keys = typeof(TKey).GetEnumValues();
            for (var i = 0; i < keys.Length; i++) {
                var key = (TKey)keys.GetValue(i);
                if (mDictTemp.ContainsKey(key)) {
                    m_dict[key] = mDictTemp[key]; 
                } else {
                    m_dict[key] = new TValue();
                }
            }
            m_keys = null;
            m_values = null;
    ...
    {
        Dictionary<TKey, TValue> mDictTemp = new Dictionary<TKey, TValue>(m_dict);
        m_dict.Clear();
        var keys = typeof(TKey).GetEnumValues();
        int i = 0;
        for (; i < keys.Length; i++) {
