...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ContentScrollSnapHorizontal : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
        ...
        private Action _onUiBeginScroll;
        ...
        private void Awake()
        {
            ...
            scrollRect.onValueChanged.AddListener(this.OnScrollValueChanged);
        }

        private void OnScrollValueChanged(Vector2 value) {
            this._onUiBeginScroll?.Invoke();
        }

        public void SetOnUiBeginDrag(Action onUiBeginScroll) {
            this._onUiBeginScroll = onUiBeginScroll;
        }
        ...
        private void SetupWithHorizontalLayoutGroup()
        {
            ...
            var scrollRectWidth = Mathf.Min(((RectTransform)contentTransform.GetChild(0)).sizeDelta.x, ((RectTransform)contentTransform.GetChild(contentTransform.childCount - 1)).sizeDelta.x);
            ...
            for (var i = 0; i < contentTransform.childCount; i++)
            {
                if (!ignoreInactiveItems || contentTransform.GetChild(i).gameObject.activeInHierarchy)
                {
                    ...
                    var offset = checkedChildrenTotalWidths + (horizLayoutGroup.spacing * activeChildrenBeforeSelf) + ((widthOfSelf - widthOfScrollRect) / 2);
        ...
        private void SetupWithCalculatedSpacing()
        {
            ...
            var totalWidth = DstFromTopLeftOfTransformToTopLeftOfParent(childFurthestToTheRight).x + childFurthestToTheRight.sizeDelta.x;

            contentTransform.sizeDelta = new Vector2(totalWidth, contentTransform.sizeDelta.y);
            var scrollRectWidth = Mathf.Min(childrenFromLeftToRight[0].sizeDelta.x, childrenFromLeftToRight[childrenFromLeftToRight.Count - 1].sizeDelta.x);
            ...
            {
                var offset = DstFromTopLeftOfTransformToTopLeftOfParent(childrenFromLeftToRight[i]).x + ((childrenFromLeftToRight[i].sizeDelta.x - widthOfScrollRect) / 2);
                scrollRect.horizontalNormalizedPosition = offset / totalScrollableWidth;
                contentPositions.Add(contentTransform.localPosition);
            }
        }
        ...
        public void GoTo(MoveInfo info)
        {
            ...
            UpdateBtPrevNext(info.index);
        }

        private void UpdateBtPrevNext(int index) {
            if (prevButton) {
                prevButton.SetActive(index > 0);
            }
            if (nextButton) {
                nextButton.SetActive(index < contentPositions.Count - 1);
            }   
        }

        private void GoToChild(int index, bool jump)
        {
            var clampedIndex = Mathf.Clamp(index, 0, contentPositions.Count - 1); //contentPositions amount == the amount of available children
                ...
                for (var i = 0; i < contentTransform.childCount; i++)
                {
                    if (!ignoreInactiveItems || contentTransform.GetChild(i).gameObject.activeInHierarchy)
                    {
                        if (availableChildIndex == clampedIndex)
                        {
                            ...
                            var offset = DstFromTopLeftOfTransformToTopLeftOfParent(startChild).x + ((startChild.sizeDelta.x - scrollRectTransform.sizeDelta.x) / 2);
                            scrollRect.horizontalNormalizedPosition = offset / totalScrollableWidth;
        ...
        private IEnumerator SlideAndLerp()
        {
            ...
            UpdateBtPrevNext(LerpTargetIndex);
        }
