...
namespace App {
    ...
    public class PlayerData {
        ...
        public bool IsLocked() {
            return timeLockSince + timeLockSeconds * 1000L > DateTime.Now.ToEpochMilliseconds();
        }
    ...
    public class BHeroManager : IBHeroManager {
        ...
        public BHeroManager(IHouseStorageManager houseStorage) {
        ...
        public void LoadMap(GameModeType _) {
        ...
        public void ReplaceOneHero(IHeroDetails detail) {
            var id = new HeroId(detail.Id, detail.AccountType);
            if (_playerDataById.TryGetValue(id, out var existing)) {
                _playerData[existing.AccountType].RemoveAll(d => d.Id == detail.Id);
            } else {
                _totalHeroes++;
            }
            _playerDataById[id] = detail;
            _playerData[detail.AccountType].Add(detail);
            UpdateHeroState(id, detail.Stage, false);
            UpdateHeroActiveState(id, detail.IsActive, false);
            RefreshSortedData();
        }
        ...
        private static bool TryGetRarityFromFilter(DialogInventory.HeroTypeFilter filter, out int rarity) {
            if (filter < DialogInventory.HeroTypeFilter.RarityCommon
                || filter > DialogInventory.HeroTypeFilter.RaritySuperMystic) {
                rarity = -1;
                return false;
            }
            rarity = (int)filter - (int)DialogInventory.HeroTypeFilter.RarityCommon;
            return true;
        }
        ...
        public (List<IHeroDetails>, int, List<int>) GetSortedPlayerDataList(
            ...
            foreach (var t in allTypes) {
                ...
                sortedList = order3 switch {
                    ...
                    _ when TryGetRarityFromFilter(order3, out var rarity) => sortedList.Where(e => e.Rarity == rarity).ToList(),
        ...
        public (List<PlayerData>, int, List<int>) GetSortedPlayerDataList(
            ...
            var result1 = order3 switch {
                ...
                _ when TryGetRarityFromFilter(order3, out var rarity) => input.Where(e => e.rare == rarity),
                _ => throw new ArgumentOutOfRangeException(nameof(order3), order3, null),
            };
            ...
            var result2 = order2 switch {
                ...
                DialogInventory.SortOrder2.HighStakeFirst =>
                    result1.OrderByDescending(e => Math.Max(e.stakeBcoin, e.stakeSen)),
                _ => throw new ArgumentOutOfRangeException(nameof(order2), order2, null)
            };
        ...
        private static PlayerType GetPlayerType(int index) {
            // Skin pool BHero 1-based (call-site truyền data.Skin - 1). enum value == skin id:
            //   1..31  -> BomberMan..FootballNinja
            //   32..58 -> Super skins (SYNC_BOMBERMAN_V4, art mượn folder HeroTR).
            // Map thẳng (PlayerType)skin, hết hardcode switch.
            var skin = index + 1;
            if (skin < 1 || skin > HeroDetails.SupportedSkinMax) {
                Debug.LogError($"Unknown BHero skin {skin}");
                return PlayerType.BomberMan;
            }
            return (PlayerType)skin;
        }
