...
namespace CustomSmartFox {
    public class EncryptedExtensionHandler<T> : IServerHandler<T> {
        ...
        public EncryptedExtensionHandler(
        ...
        async Task<T> IServerHandler<T>.Start(IServerBridge bridge) {
        ...
        public void OnExtensionResponse(string cmd, int requestId, byte[] data) {
            ...
            try {
                var (outData, json) = _encoder.DecodeData<T>(data);
                Log(false, json);
