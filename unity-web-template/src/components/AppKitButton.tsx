...
const AppKitButton: React.FC<AppKitButtonProps> = () => {
    ...
    const handleConnectWallet = async () => {
        try {
            // Call the continueConnection method on UnityCommunicator
    ...
    // Render custom UI if showCustomUI is true
    ...
    // Show the fake connect button if showFakeConnect is true
    if (state.showFakeConnect) {
        return (
            <button
                style={styles.connectButtonFake}
                onMouseOver={(e) => {
                    e.currentTarget.style.backgroundColor = '#3a5dca';
                }}
                onMouseOut={(e) => {
                    e.currentTarget.style.backgroundColor = '#5773ff';
                }}
    ...
    // Otherwise show the real appkit button
