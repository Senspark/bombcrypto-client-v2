...
export default class ApiService {
    ...
    public setApiHost(networkType: SupportedNetwork | undefined | null) {
        // Set default
        ...
        if (networkType === 'bsc') {
            this._curApiHost = this._apiHost.replace('web', 'web/bsc');
        }
        if (networkType === 'polygon') {
            this._curApiHost = this._apiHost.replace('web', 'web/pol');
        }
    }
    ...
    public async changeNickName(userName: string, newNickName: string): Promise<boolean> {
        try {
            const path = `${this._curApiHost}/change_nick_name`;
            const body = JSON.stringify({
                userName: userName,
