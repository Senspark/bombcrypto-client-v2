...
function initialize(logger:Logger, communicator: UnityCommunicator) {
    ...
    methods.set('GET_CONNECTION', communicator.getConnection.bind(communicator));
    ...
    methods.set('GET_NETWORK_DATA', communicator.getNetworkData.bind(communicator));
...
function setUnityInput(state: boolean) {
    if (window.unityInstance) {
        _communicator?.sendMessageToUnityNoEncrypt(UnityCommand.SET_INPUT, String(state));
    } else {
        // console.error('Unity instance is not initialized');
    }
}
...
function skipReloadClient() {
    _skipReloadClient = true;
}
